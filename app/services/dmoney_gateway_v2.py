...
class DmoneyPaymentGateway:
    ...
    def __init__(self):
        ...
        self.checkout_base_url = (settings.DMONEY_CHECKOUT_BASE_URL or self.DEFAULT_CHECKOUT_BASE).strip()
    ...
    def _api_url(self, path: str) -> str:
        """Build full API URL, handles base_url that may already contain gateway path."""
        base = self.base_url
        if self.GATEWAY_PATH in base:
            base = base[: base.index(self.GATEWAY_PATH)]
        if not path.startswith("/"):
            path = "/" + path
        return f"{base}{self.GATEWAY_PATH}{path}"
    ...
    def _parse_expiry(self, expiry_str: str) -> float:
        ...
        for fmt in self._EXPIRY_FORMATS:
            try:
                ...
                continue
        logger.warning(f"⚠️  Could not parse expirationDate '{expiry_str}' — defaulting to +1 hour")
    ...
    def get_token(self) -> Dict:
        ...
        self.token_expiry = self._parse_expiry(expiry_str) if expiry_str else time.time() + 3600
    ...
    def create_preorder(
        ...
        """Step 2: Create a PreOrder at D-Money."""
        self._ensure_token()
    ...
    def generate_checkout_url(self, prepay_id: str, language: str = "en") -> str:
        """Step 3: Build the checkout URL for the customer's browser."""
        nonce_str = self._nonce()
        timestamp = self._timestamp()
        ...
        url = f"{self.checkout_base_url}?{urllib.parse.urlencode(query_params)}"
    ...
    def create_payment(
        ...
        """High-level helper: PreOrder + Checkout URL in one call."""
        order_id = order_id or self._generate_order_id()

        raw = self.create_preorder(
            amount=amount, title=title, order_id=order_id,
